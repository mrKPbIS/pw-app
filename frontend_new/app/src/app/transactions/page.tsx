      ...
      <Container component="main" maxWidth="sm" sx={{ mb: 4 }}>
        <Paper variant="outlined" sx={{ my: { xs: 3, md: 6 }, p: { xs: 2, md: 3 } }}>
          ...
          <List>
            {
              transactions.length
              ? transactions.map((item) =>{ return (<TransactionsItem key={item.id} transaction={item} />);})
              : <Typography variant="h6" component="div">No transactions</Typography>
            }
  ...
  if (userId === owner.id) {
    ...
    elem = (
      <ListItem>
        ...
        <Typography align='right' color='green'>+{amount}</Typography>
      </ListItem>
    );
  }
  return elem;
