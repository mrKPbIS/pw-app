...
import { Box, Container, AppBar, Button, Typography, Paper, Toolbar, ListItem, TextField, List, ListItemText, Autocomplete } from '@mui/material';
...
export default function TransactionCreateForm(props: any) {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (req.success && req.data) {
        const list = req.data.users.map(({ id, name}) => { return { label: name, id }});
        console.log(list === usersList, usersList, list)
        setUsersList(list);
      }
    }
  ...
  const handleSubmit = async (event: SyntheticEvent) => {
    ...
    const req = await createTransaction({ amount, recipientId: recipient }, token);
    if (req.success && req.data) {
      router.push('/profile');
    }
  }
            ...
            <List>
              ...
              <ListItem>
                ...
                <Autocomplete
                  ...
                  options={usersList}
                  onChange={(e, value) => { setRecipient(value.id)}}
                  renderInput={(params) => <TextField {...params} label="Recipient"/>}
                />
              </ListItem>
              <ListItem>
                <ListItemText primary="Amount"/>
                <TextField id='transaction-amount' label='amount' variant="filled" value={amount} onChange={(e) => setAmount(e.target.value)}/>
              </ListItem>
